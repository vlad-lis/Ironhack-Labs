...
SELECT district_id AS id, COUNT(client_id) AS 'count'
FROM `client`
...
SELECT account_id, sum(amount) AS 'sum of loans'
FROM loan
GROUP BY account_id
...
SELECT `date`, duration, count(amount) as 'count'
FROM loan
WHERE 
	left(`date`, 4) = '9712' and amount != 0
...
SELECT account_id, `type`, sum(amount) as 'sum'
FROM trans
...
SELECT 
	account_id as id, 
	CASE WHEN `type` = 'PRIJEM' THEN 'Incoming' ELSE 'Outgoing' END AS 'type',
    floor(sum(amount)) as 'sum'
FROM trans
WHERE account_id = 396
GROUP BY account_id, `type`
ORDER BY `type`;
